# Apps

Application and demo workspaces live here.

Current demos:

- `angular-demo/`: Angular-backed demo that mounts the `@thefoolspath/gethen-angular` adapter.
- `core-demo/`: framework-neutral demo for the core virtualized DOM renderer.

Current research prototypes:

- `renderer-canvas-prototype/`: dependency-free Canvas 2D renderer prototype for comparison against the DOM renderer.
- `renderer-prototype/`: dependency-free virtualized DOM renderer prototype for the pre-alpha research gate.
