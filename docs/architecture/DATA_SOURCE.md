...
Initial implementation: `packages/core/src/client-data-source.ts` provides client-side row ID extraction, range retrieval, and stale-safe cell updates. Client-side execution is the only runtime scope through 1.0. Server DataSource, its wire protocol, cache semantics, and backend integration move to 2.0.
