...
Status: Proposed. No production implementation exists yet.
...
| `0.0.0-alpha.2` | Interaction basics and adapter completion | Deferred first adapter if alpha.1 used only a core demo, range selection, clipboard copy/paste basics, validation primitives, improved column/cell metadata | Keyboard interaction benchmark, clipboard payload sanity checks, adapter mount/unmount tests, accessibility smoke tests |
...
| --- | --- | --- |
| Angular adapter | Conditional | Add only after the first adapter/core API is stable enough. |
