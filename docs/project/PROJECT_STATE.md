...
- Emerald Light and Emerald Dark semantic documentation modes with system-preference initialization, persisted explicit selection, an accessible theme toggle, route search, a single collapsible icon-and-text hierarchical sidebar, context bar, desktop page table of contents, page navigation, and documentation footer.
- Public readonly `gethenLightTheme` and `gethenDarkTheme` Grid presets with complete existing visual-token mappings, object-spread customization, unchanged unthemed defaults, and Angular theme-only updates that preserve the mounted Grid instance.
...
- Initial TypeScript reference engine in `packages/core/src/state/client-grid-engine.ts`.
- Initial virtualized DOM renderer slice, selection/navigation, editing, framework-neutral core demo, and client DataSource in `packages/core/src/` and `apps/core-demo/`; Core source is grouped by responsibility and renderer cell DOM creation and value coercion helpers remain split into focused modules.
...
- Repository-local visual reference for the accepted Emerald Intelligence direction: [../product/assets/emerald-intelligence-brand-style-guide.png](../product/assets/emerald-intelligence-brand-style-guide.png). It is documentation reference material and is not shipped by the docs application.
...
- Implemented Gethen Corporate Identity and light/dark Grid theme-system plan; browser and manual visual verification remain open pending explicit approval: [../plans/active/0008-corporate-identity-and-theme-system.md](../plans/active/0008-corporate-identity-and-theme-system.md).
- Corporate identity guide: [../product/CORPORATE_IDENTITY.md](../product/CORPORATE_IDENTITY.md).
...
| 2026-08-07 | Added initial TypeScript reference engine with tests and core benchmark | `packages/core/src/state/client-grid-engine.ts`, `packages/core/src/state/client-grid-engine.test.ts`, `benchmarks/typescript-reference/core-engine-operations.mjs` |
...
| 2026-08-08 | Fixed core renderer cells disappearing after vertical scrolling and added browser regression coverage | `packages/core/src/renderer/dom/virtual-dom-grid.ts`, `tests/browser/renderer-prototypes.spec.ts`, [ISSUE_AND_CHANGE_REQUESTS.md](ISSUE_AND_CHANGE_REQUESTS.md) |
| 2026-08-08 | Fixed double-click editing, type-to-edit, and browser-test port conflicts with browser regression coverage | `packages/core/src/renderer/dom/virtual-dom-grid.ts`, `scripts/run-browser-tests.mjs`, `tests/browser/renderer-prototypes.spec.ts`, [ISSUE_AND_CHANGE_REQUESTS.md](ISSUE_AND_CHANGE_REQUESTS.md) |
| 2026-08-10 | Refactored package renderer and adapter source for clearer separation of responsibilities and split Angular template/CSS assets from TypeScript | `packages/core/src/renderer/dom/virtual-dom-grid.ts`, `packages/core/src/renderer/dom/virtual-dom-grid-cell.ts`, `packages/core/src/renderer/dom/virtual-dom-grid-values.ts`, `packages/gethen-angular/src/gethen-grid.component.ts`, `packages/gethen-angular/src/gethen-grid.component.html`, `packages/gethen-angular/src/gethen-grid.component.css`, `scripts/copy-package-assets.mjs` |
| 2026-08-10 | Started Alpha 2 with typed view metadata, hidden rendered columns, conditional application classes, text formatters, theme tokens, Angular passthrough, and regression coverage | `packages/core/src/renderer/dom/grid-customization.ts`, `packages/core/src/renderer/dom/virtual-dom-grid.ts`, `packages/gethen-angular/src/gethen-grid.component.ts`, [../product/CUSTOMIZATION.md](../product/CUSTOMIZATION.md), [../plans/completed/0003-developer-customization-and-row-transactions.md](../plans/completed/0003-developer-customization-and-row-transactions.md) |
| 2026-08-10 | Added Alpha 2 rectangular range selection with keyboard and pointer extension, normalized range events, ARIA state, Angular passthrough, and browser coverage | `packages/core/src/renderer/dom/virtual-dom-grid.ts`, `packages/core/src/renderer/dom/virtual-dom-grid-cell.ts`, `packages/gethen-angular/src/gethen-grid.component.ts`, `tests/browser/renderer-prototypes.spec.ts` |
| 2026-08-10 | Added Alpha 2 DTO mapping and headless row transactions with stable identity, runtime metadata checks, dirty-field tracking, row save payloads, and unit coverage | `packages/core/src/data/grid-model.ts`, `packages/core/src/data/row-transactions.ts`, `packages/core/src/data/grid-model.test.ts`, `packages/core/src/data/row-transactions.test.ts` |
| 2026-08-10 | Added Alpha 2 opt-in validated direct clipboard paste, per-cell errors, all-or-nothing commit, pasted-range selection, host-dialog preparation, Angular passthrough, and regression coverage | `packages/core/src/state/grid-clipboard.ts`, `packages/core/src/renderer/dom/virtual-dom-grid.ts`, `packages/gethen-angular/src/gethen-grid.component.ts`, `tests/browser/renderer-prototypes.spec.ts` |
...
| 2026-08-10 | Implemented the automated Alpha 3 editing, trusted extension, bounded history, layout/frozen-pane, Angular registry, and worker-boundary scope; manual NVDA/Chrome remains open | `packages/core/src/state/grid-editing.ts`, `packages/core/src/state/grid-history.ts`, `packages/core/src/state/grid-layout.ts`, `packages/gethen-angular/src/gethen-angular-registry.ts`, `benchmarks/engine-bakeoff/`, [ALPHA_3_RELEASE_NOTES.md](ALPHA_3_RELEASE_NOTES.md) |
| 2026-08-10 | Added the first Alpha 4 checkpoint: canonical shaping, mixed-type columnar worker contract, TypeScript and Rust/WASM candidates, browser parity, cancellation, and diagnostic numeric boundary evidence; engine selection remains open | `packages/core/src/shaping/grid-data-shaping.ts`, `packages/core/src/contracts/engine-contract.ts`, `crates/gethen-engine/src/lib.rs`, `benchmarks/engine-bakeoff/`, [../research/findings/2026-08-10-alpha4-worker-boundary-checkpoint.md](../research/findings/2026-08-10-alpha4-worker-boundary-checkpoint.md) |
...
| 2026-09-01 | Accepted the initial Gethen Corporate Identity and theme-system direction: Frost Light and Winter Night, public overridable Grid presets, unchanged no-theme Grid defaults, Aspire-inspired documentation UX without copying its brand, and explicit approval required before Browser, Chrome, Computer Use, or browser rendering. This direction was superseded by Emerald Intelligence on 2026-09-07. | [../plans/active/0008-corporate-identity-and-theme-system.md](../plans/active/0008-corporate-identity-and-theme-system.md), [../project-name.md](../project-name.md) |
