...
- `engine/`: the production TypeScript Worker plus the internal Rust/WASM parity/benchmark oracle.
- `renderer/dom/`: virtualized DOM rendering, viewport, shell, and customization.

Only `core/src/index.ts` is a public barrel. It exposes the implementation-neutral
`createGridWorkerEngine()` production factory; candidate-specific factories remain internal.
Internal modules use direct relative imports.
